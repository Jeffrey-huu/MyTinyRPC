...
namespace MyTinyRPC {
	...
	void RpcChannel::termCallback() {
		...
		if (m_closure) {
			// DEBUGLOG("call the closure callback")
			m_closure->Run();
			if (my_controller) {
	...
	void RpcChannel::CallMethod(const google::protobuf::MethodDescriptor* method,
		...
		std::shared_ptr<MyTinyRPC::TinyPBProtocol> req_protocol = std::make_shared<MyTinyRPC::TinyPBProtocol>();
		...
		m_client->connect([req_protocol, this]() mutable {
			...
			getTcpClient()->readMessage(req_protocol->m_msg_id, [this, my_controller](AbstractProtocol::s_ptr msg) mutable {
				std::shared_ptr<MyTinyRPC::TinyPBProtocol> rsp_protocol = std::dynamic_pointer_cast<MyTinyRPC::TinyPBProtocol>(msg);
